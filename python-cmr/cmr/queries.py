...
class Query(object):
    ...
    def __init__(self, route, mode=CMR_OPS, export_path=""):
        self.params = {}
        self.options = {}
        self._route = route
        self.mode(mode)
        self.export_path = export_path # a directory path
        # if export path is set, it will auto-export to the path with 2000 limit object per file.

    def get(self, limit=2000, start_page=1):
        """
    ...
    def get_all(self, start_page=1):
        ...
        :returns: query results as a list
        """
        return self.get(self.hits(), start_page)
    ...
    def mode(self, mode=CMR_OPS):
        ...
        print("Initializing Query with", self._base_url)
...
class GranuleQuery(Query):
    ...
    def __init__(self, mode="CMR_OPS", export_path=""):
        if mode == "CMR_UAT":
            mode = CMR_UAT
        elif mode == "CMR_SIT":
            mode = CMR_SIT
            ...
            # default CMR_OPS
            mode = CMR_OPS
        Query.__init__(self, "granules", mode, export_path)
...
class CollectionQuery(Query):
    ...
    def __init__(self, mode="CMR_OPS", export_path=""):
        if mode == "CMR_UAT":
            ...
            # default CMR_OPS
            mode = CMR_OPS
        Query.__init__(self, "collections", mode, export_path)
